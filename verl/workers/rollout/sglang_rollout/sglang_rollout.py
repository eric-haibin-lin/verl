...
class SGLangRollout(BaseRollout):
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"tool_schemas: {self._tool_schemas}, tool_map: {self._tool_map}, "
            f"tool_call_parser_type: {self._tool_call_parser_type}, sgl_tools: {self._sgl_tools}, "
            f"function_call_parser: {self._function_call_parser}"
        )
        ...
        **kwargs,
        ...
        while current_turns < self.config.multi_turn.max_assistant_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                # since SGLang raises an error when max_new_tokens + 1 is greater to max_model_len
                # (the extra token accounts for the EOS token).
