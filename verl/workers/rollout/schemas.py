...
class AsyncRolloutRequest(BaseModel):
    ...
    def initialize_request(cls, values):
        ...
        if not values.get("input_ids") or not values.get("attention_mask"):
            ...
            if len(values["input_ids"]) > max_prompt_len:
                ...
                logger.warning(
                    f"Prompt {values['batch_data_id']} length {len(values['input_ids'])} greater than "
                    f"max_prompt_len {max_prompt_len} after applied chat template with tools."
                )
    ...
    def add_user_message(
        ...
        # We don't need to pass multi_modal_data here because we don't have any multi-modal data
        # from Engine Inference, it is pure text.
    ...
    def finalize(
        ...
        if self.tokenization_sanity_check_mode != TokenizationSanityCheckModeEnum.OFF:
            ...
            if diffs := self._get_prompt_diffs(
                ...
                if log_warning:
                    ...
                    logger.warning(
                        f"Showing {diff_surrounding_chars} characters before and after the diffs for context "
                        f"and better readability."
                    )
                    diff_details_list = []
                    for d in diffs:
        ...
        # In case we failed to generate the assistant message and the generation prompt ids were already
        # added to input_ids, remove them from the end of input_ids
        if self.input_ids[-len(self.generation_prompt_ids) :] == self.generation_prompt_ids:
