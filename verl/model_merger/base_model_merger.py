...
def parse_args():
    ...
    base_op_parser.add_argument(
        "--local_dir", type=str, default=None, help="Path to the original Hugging Face model for config."
    )
    base_op_parser.add_argument(
        "--tie-word-embedding",
        action="store_true",
    ...
    base_op_parser.add_argument(
        ...
        action="store_true",
        help="Whether to use CPU initialization for the model. This is useful for large models that cannot fit "
        "into GPU memory during initialization.",
    )
    ...
    def patch_model_generation_config(self, model):
        ...
        if model.can_generate():
            try:
                ...
                print(
                    f"Warning: Generation config file not found in {self.hf_model_config_path}, using a generation "
                    f"config created from the model config."
                )
        return model
