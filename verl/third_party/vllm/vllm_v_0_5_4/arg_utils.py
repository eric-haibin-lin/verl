...
class EngineArgs:
    ...
    def add_cli_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            "--model", type=str, default="facebook/opt-125m", help="name or path of the huggingface model to use"
        )
        parser.add_argument(
            "--tokenizer",
            type=str,
        ...
        parser.add_argument(
            ...
            choices=["auto", "slow"],
            help='tokenizer mode. "auto" will use the fast tokenizer if available, and "slow" will always use '
            "the slow tokenizer.",
        )
        ...
        parser.add_argument(
            ...
            help='data type for model weights and activations. The "auto" option will use FP16 precision for '
            "FP32 and FP16 models, and BF16 precision for BF16 models.",
        )
        parser.add_argument(
            "--max-model-len",
            type=int,
        ...
        # KV cache arguments
        parser.add_argument(
            "--block-size", type=int, default=EngineArgs.block_size, choices=[8, 16, 32], help="token block size"
        )
        # TODO(woosuk): Support fine-grained seeds (e.g., seed per request).
        parser.add_argument("--seed", type=int, default=EngineArgs.seed, help="random seed")
        parser.add_argument(
            "--swap-space", type=int, default=EngineArgs.swap_space, help="CPU swap space size (GiB) per GPU"
        )
        parser.add_argument(
    ...
    def create_engine_config(
        ...
        if (
            ...
            raise ValueError(
                f"BitsAndBytes quantization and QLoRA adapter only support 'bitsandbytes' load format, "
        ...
        if (
            ...
            raise ValueError(
                f"BitsAndBytes load format and QLoRA adapter only support 'bitsandbytes' quantization, "
                f"but got {self.quantization}"
            )
        ...
        if self.enable_chunked_prefill is None:
            ...
            if use_long_context:
                ...
                if (
                    ...
                    logger.warning(
                        "Chunked prefill is enabled by default for models with max_model_len > 32K. Currently, "
                        "chunked prefill might not work with some features or models. If you encounter any issues, "
                        "please disable chunked prefill by setting --enable-chunked-prefill=False."
                    )
            if self.enable_chunked_prefill is None:
        ...
        if not self.enable_chunked_prefill and use_long_context:
            logger.warning(
                "The model has a long context length (%s). This may cause OOM errors during the initial "
                "memory profiling phase, or result in low performance due to small KV cache space. Consider "
        ...
        scheduler_config = SchedulerConfig(
            ...
            use_v2_block_manager=self.use_v2_block_manager,
            num_lookahead_slots=(
                self.num_lookahead_slots if speculative_config is None else speculative_config.num_lookahead_slots
            ),
        ...
        prompt_adapter_config = (
            PromptAdapterConfig(
                max_prompt_adapters=self.max_prompt_adapters, max_prompt_adapter_token=self.max_prompt_adapter_token
            )
        ...
        if (
            ...
            raise ValueError(
                "Chunked prefill is not supported with sliding window. Set --disable-sliding-window to "
