...
class DataProto:
    ...
    def check_consistency(self):
        ...
        if self.batch is not None and self.non_tensor_batch is not None and len(self.non_tensor_batch) != 0:
            ...
            for key, val in self.non_tensor_batch.items():
                assert isinstance(val, np.ndarray), (
                    f"data in the non_tensor_batch must be a numpy.array with dtype=object, but for {key=}, "
                    f"got {type(val)=}"
                )
    ...
    def make_iterator(self, mini_batch_size, epochs, seed=None, dataloader_kwargs=None):
        ...
        Args:
            ...
            dataloader_kwargs (Any): internally, it returns a DataLoader over the batch. The dataloader_kwargs is the
                kwargs passed to the DataLoader.
