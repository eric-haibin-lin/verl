...
class RaySPINTrainer:
    ...
    def fit_dpo(self):  # Renamed for clarity as standard PPO loop
        ...
        if not self.use_reference_policy:
            print(
                "WARNING: 'use_reference_policy' is False. Periodic reference model update requires a reference "
                "policy worker. DPO updates might fail or use incorrect logic."
            )
            # Consider raising an error if strict adherence is required:
            # raise ValueError("Periodic reference model update requires 'use_reference_policy' to be True and "
            #                  "a configured reference worker.")
                ...
                try:  # Outer try-except for the whole step
                    ...
                    with _timer("step", timing_raw):
                        ...
                        if self.use_reference_policy:
                            ...
                            print(
                                "Warning: Skipping external reference log prob calculation as "
                                "use_reference_policy is False."
                            )
