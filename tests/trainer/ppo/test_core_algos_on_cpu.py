...
from verl.trainer.ppo.core_algos import compute_gae_advantage_return, compute_grpo_outcome_advantage, compute_policy_loss, get_adv_estimator_fn, register_adv_est
...
class TestComputeGAEAdvantageReturn(unittest.TestCase):
    ...
    def test_short_response(self):
        ...
        token_level_rewards, values, response_mask, gamma, lam = self._create_test_tensors(response_length=2)
...
class TestComputeGRPOOutcomeAdvantage(unittest.TestCase):
    ...
    def test_norm_adv_by_std_flag(self):
        ...
        advantages_norm, returns_norm = compute_grpo_outcome_advantage(token_level_rewards, response_mask, index, norm_adv_by_std_in_grpo=True)

        advantages_no_norm, returns_no_norm = compute_grpo_outcome_advantage(token_level_rewards, response_mask, index, norm_adv_by_std_in_grpo=False)
    ...
    def test_epsilon_parameter(self):
        ...
        advantages, returns = compute_grpo_outcome_advantage(token_level_rewards, response_mask, index, epsilon=1e-8)
...
class TestComputePolicyLoss(unittest.TestCase):
    ...
    def test_basic_functionality(self):
        ...
        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_policy_loss(old_log_prob, log_prob, advantages, response_mask, cliprange=0.2)
    ...
    def test_different_loss_agg_modes(self):
        ...
        for mode in modes:
            pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_policy_loss(old_log_prob, log_prob, advantages, response_mask, cliprange=0.2, loss_agg_mode=mode)
    ...
    def test_dual_clip_parameters(self):
        ...
        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_policy_loss(old_log_prob, log_prob, advantages, response_mask, cliprange_low=0.1, cliprange_high=0.3, clip_ratio_c=2.0)
    ...
    def test_cliprange_parameter_validation(self):
        ...
        for cliprange in [0.1, 0.2, 0.5]:
            pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_policy_loss(old_log_prob, log_prob, advantages, response_mask, cliprange=cliprange)
