...
class CustomCompletionCallback(ToolCompletionCallback):
    ...
    async def __call__(self, messages: List[Dict[str, str]], completions: ChatCompletion, info: Dict[str, Any]):
        ...
        if metadata["run_status"] != "Finished":
            print(
                f"[id={completions.id},turn={turn},finish_reason={finish_reason}] "
                f"Code block execution failed: {metadata}, done!"
            )
            return
...
3. User will send the output in format <interpreter>output</interpreter> to you, and you should use the output to 
answer the question.
