...
def test_integration_concurrency_high_load():
    """Integration test: High concurrency (100 cases) against real API with mixed results (success, wrong answer,
    timeout)"""
...
def test_multiprocess_global_concurrency_limit_with_semaphore():
    ...
    assert total_execution_time >= min_expected_duration * 0.8, (
        f"Total execution time ({total_execution_time:.2f}s) was unexpectedly short, suggesting the semaphore "
        f"might not be effectively limiting concurrency as expected (min expected: {min_expected_duration * 0.8:.2f}s)."
    )
